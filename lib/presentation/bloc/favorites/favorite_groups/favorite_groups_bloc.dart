...
class FavoriteGroupsBloc extends Bloc<IFavoriteGroupsEvent, IFavoriteGroupsState> {
  FavoriteGroupsBloc() : super(const FavoriteGroupsInitial()) {
    on<LoadFavoriteGroupsEvent>(_load);
  }

  FutureOr<void> _load(LoadFavoriteGroupsEvent event, Emitter<IFavoriteGroupsState> emit) async {
    emit(const FavoriteGroupsLoading());
    try {
      final usecase = locator<IUseCase<BaseModel<FavoriteGroupsModel>?, Null>>(instanceName: 'ListFavoriteGroups');
